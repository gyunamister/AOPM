						...
						for(String task : r.tasks.keySet()) {
							if(rot.get(i).process.next_act.equals(task)) {
								...
								if(rot.get(index) instanceof Order) {
									...
									e = rot.get(index).process.recordEvent(eventlog, ((Item)rot.get(index)).order, (Item)rot.get(index), null, null, 
											rot.get(index).process.current_act, r, rot.get(index).available_at, e);
								}else if(rot.get(index) instanceof Package) {
									e = rot.get(index).process.recordEvent(eventlog, ((Package)rot.get(index)).order, null, (Package)rot.get(index), null, 
											rot.get(index).process.current_act, r, rot.get(index).available_at, e);
								}else if(rot.get(index) instanceof Route) {
									e = rot.get(index).process.recordEvent(eventlog, ((Route)rot.get(index)).order, null, null, (Route)rot.get(index), 
											rot.get(index).process.current_act, r, rot.get(index).available_at, e);
								}else {
